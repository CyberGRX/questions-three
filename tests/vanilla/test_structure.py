...
ARBITRARY_DICT = {"things": [1, 2, 3], "stuffs": {"part1": [5, 6], "part2": "The Spinach Imposition"}}
...
class TestStructure(TestCase):
    ...
    def test_returns_level_three(self):
        spam_count = 0x42
        things = {"one": {"two": {"spams": spam_count}}}
    ...
    def test_can_retrieve_keys_as_sequence(self):
        ...
        keys = sut.keys()
        assert hasattr(keys, "__iter__"), "keys() returned a %s. Expected a sequence type" % type(keys)
    ...
    def test_can_retrieve_values_as_sequence(self):
        ...
        values = sut.values()
        assert hasattr(values, "__iter__"), "values() returned a %s.  Expected a sequence type" % type(values)
    ...
    def test_test_can_retrieve_items_as_sequence_of_tuples(self):
        ...
        items = sut.items()
        assert hasattr(items, "__iter__"), "items() returned a %s.  Expected a sequence type" % type(items)
    ...
    def test_to_and_from_json_dict(self):
        ...
        expect(json.loads(Structure.from_json(serialized).to_json())).to(equal(ARBITRARY_DICT))
