...
from questions_three.vanilla import \
    Structure, structures_from_json, structures_to_json
...
class TestListOfStructures(TestCase):
    ...
    def test_to_and_from_json(self):
        original = [
            Structure(spam=Structure(eggs=1, sausage=['spam'])),
            Structure(larry=1, moe=3, curly=0)]
        expect(structures_from_json(structures_to_json(original))).to(
            equal(original))
