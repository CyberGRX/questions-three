...
class TestOperationFailed(TestCase):
    ...
    @parameterized.expand(["this is what were looking for", "these are not the droids youre looking for"])
    ...
    @parameterized.expand([({"vari": 1, "able": 2},), ({"time": 1, "step": 2},)])
    ...
    @parameterized.expand([({"expected": "things"},), ({"expected": "dings"},)])
    ...
    @parameterized.expand(
        [([{"message": "youre bad at requests"}],), ([{"message": "heres a 400", "locations": []}],)]
    )
    ...
    @parameterized.expand(["You wrote this test bad", "shaaaame, shame on you"])
    ...
    @parameterized.expand(
        [(("this is a custom message",),), (("this is the first message", "this is the second message"),)]
    )
