...
class TestExecute(TestCase):
    ...
    @parameterized.expand([({"dingle": "dangle"},), ({"dangle": ["dingle", "dongle"]},)])
    ...
    def test_graph_ql_response_object_is_passed_response_from_operation(self):
        ...
        assert response.http_response == self.fake_requests_response, (
            f"Was not passed the fake_requests_response " f"with url: '{expected_url}'"
        )
    ...
    @parameterized.expand(
        [("str", "this is a string with the word errors in it"), ("list", ["errors", "this should not happen"])]
    )
