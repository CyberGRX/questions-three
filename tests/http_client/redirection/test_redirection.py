...
class TestRedirection(TestCase):
    ...
    def test_repeats_initial_request_headers(self):
        ...
        expect(self.spies.get.kwargs_from_last_call()).to(have_keys(headers=headers))
    ...
    def test_repeats_initial_request_data(self):
        ...
        expect(self.spies.get.kwargs_from_last_call()).to(have_keys(data=data))
    ...
    def check_follows_absolute_location(self, status_code):
        ...
        expect(self.spies.get.args_from_last_call()[0]).to(equal(location))
    ...
    def test_does_not_handle_305(self):
        ...
        self.queue_response(status_code=200)
        client = HttpClient()
        expect(lambda: client.get("http://something")).to(complain(HttpUseProxy))
    ...
    def test_complains_about_infinite_redirects(self):
        self.queue_response(status_code=301, location="http://strawberry-fields")
        client = HttpClient()
        expect(lambda: client.get("http://somewhere")).to(complain(TooManyRedirects))

    def test_complains_if_response_lacks_location_header(self):
        self.queue_response(status_code=301, location=None)
        self.queue_response(status_code=200)
        client = HttpClient()
        expect(lambda: client.get("http://it")).to(complain(InvalidHttpResponse))
    ...
    def test_follows_relative_location_url(self):
        ...
        expect(self.spies.get.args_from_last_call()[0]).to(
            equal(url_append(f"{original_parsed.scheme}://{original_parsed.netloc}", redirect))
        )
    ...
    def test_preserves_fragment_if_none_specified_in_location_header(self):
        ...
        expect(self.spies.get.args_from_last_call()[0]).to(
            equal(
                url_append(f"{original_parsed.scheme}://{original_parsed.netloc}", redirect)
                + f"#{original_parsed.fragment}"
            )
        )
