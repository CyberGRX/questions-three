...
from questions_three.reporters.result_compiler.suite_results import SuiteResults
...
class TestExtendedSuiteName(TestCase):
    ...
    def test_prepends_path_to_suite_name(self):
        path = "one/two/three"
        self.context.inject(path_to_entry_script, lambda: os.path.join(path, "test_tea.py"))
    ...
    def test_masks_path_to_workspace_if_set(self):
        ...
        self.context.inject(path_to_entry_script, lambda: os.path.join(workspace_path, relative_path, "my_test.py"))
        self.publish_results()
        report = self.retrieve_report()
    ...
    def test_masks_path_to_cwd_if_workspace_unset(self):
        ...
        self.context.inject(path_to_entry_script, lambda: os.path.join(fake_cwd, relative_path, "whatever.py"))
        self.publish_results()
        report = self.retrieve_report()
        prefix = relative_path.replace("/", ".")
        expect(report.name).to(equal(prefix + "." + suite_name))
