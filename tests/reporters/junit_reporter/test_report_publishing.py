...
from questions_three.reporters.junit_reporter.junit_reporter import JunitReporter
...
class TestReportPublishing(TestCase):
    def setUp(self):
        ...
        EventBroker.subscribe(event=TestEvent.report_created, func=self.capture_published_kwargs)
    ...
    def publish_results(self):
        ...
        self.results.tests = [TestResult(test_name="Test %d" % n) for n in range(4)]
    ...
    def test_names_file_after_suite(self):
        ...
        expect(self.published_kwargs).to(contain_key_with_value("report_filename", "%s.xml" % self.suite_name))

    def test_names_nameless_suite(self):
        self.suite_name = None
        self.publish_results()
        expect(self.published_kwargs).to(contain_key_with_value("report_filename", "NamelessSuite.xml"))
