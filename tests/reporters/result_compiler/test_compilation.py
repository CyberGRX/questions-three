...
class Subscriber:
    def __init__(self):
        self.results = None
        EventBroker.subscribe(event=TestEvent.suite_results_compiled, func=self.receiver)
...
def err_test(test_name, exception=None):
    EventBroker.publish(event=TestEvent.test_erred, test_name=test_name, exception=exception)


def fail_test(test_name, exception=None):
    EventBroker.publish(event=TestEvent.test_failed, test_name=test_name, exception=exception)


def skip_test(test_name, exception=None):
    EventBroker.publish(event=TestEvent.test_skipped, test_name=test_name, exception=exception)
...
class TestResultCompiler(TestCase):
    ...
    def test_handles_test_failed_event_without_exception_gracefully(self):
        ...
        def attempt():
            EventBroker.publish(event=TestEvent.test_failed, test_name=name, exception=RuntimeError())
    ...
    def test_attaches_exception_to_errant_suite(self):
        ...
        EventBroker.publish(event=TestEvent.suite_erred, suite_name=suite_name, exception=exception)
        end_suite()
    ...
    def test_creates_suite_exception_if_none_specified(self):
        ...
        start_suite(suite_name)
        EventBroker.publish(event=TestEvent.suite_erred, suite_name=suite_name, exception=RuntimeError())
        end_suite()
    ...
    def test_attaches_artifact_to_named_test(self):
        ...
        start_suite()
        start_test(name)
        EventBroker.publish(event=TestEvent.artifact_created, test_name=name, artifact=artifact)
        end_test(name)
        end_suite()
        results = self.retrieve_results()
        expect(results.tests).to(have_length(1))
    ...
    def test_attaches_artifact_to_suite_when_no_test_named(self):
        ...
        start_suite()
        EventBroker.publish(event=TestEvent.artifact_created, artifact=artifact)
        end_suite()
        results = self.retrieve_results()
    ...
    def test_creates_missing_test_on_artifact_with_test_name(self):
        test_name = "oops"
        start_suite()
        EventBroker.publish(event=TestEvent.artifact_created, test_name=test_name, artifact="this")
        end_suite()
        results = self.retrieve_results()
        expect(results.tests).to(have_length(1))
        test = results.tests[0]
