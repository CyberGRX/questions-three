...
def execute(func=lambda item: None):
    execute_test_table(
        table=(
                ...
                ('spam', TEST_NAMES[0]),
                ('eggs', TEST_NAMES[1])),
        func=func, suite_name=SUITE_NAME)


def events_with_type(call_history, specified_type):
    return [
        published for published in call_history
        if published[1]['event'] == specified_type]
...
class TestEventPublishing(TestCase):
    ...
    def test_publishes_suite_started_exactly_once(self):
        ...
        EventBroker.subscribe(events=ALL_EVENTS, func=spy)
        execute()
        expect(events_with_type(spy.call_history, TestEvent.suite_started)).to(
            have_length(1))
    ...
    def test_publishes_suite_ended_exactly_once(self):
        ...
        EventBroker.subscribe(events=ALL_EVENTS, func=spy)
        execute()
        expect(events_with_type(spy.call_history, TestEvent.suite_ended)).to(
            have_length(1))
    ...
    def test_pubishes_test_started_with_test_name(self):
        ...
        execute()
        expect(
            [call[1]['test_name'] for call in spy.call_history]).to(
                equal(list(TEST_NAMES)))
    ...
    def test_publishes_sample_measured_with_execution_time(self):
        ...
        EventBroker.subscribe(
            event=TestEvent.test_started, func=start_spy)
        EventBroker.subscribe(
            event=TestEvent.sample_measured, func=measure_spy)
        execute()
