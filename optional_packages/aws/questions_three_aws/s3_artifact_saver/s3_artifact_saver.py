...
from questions_three.reporters.artifact_saver.artifact_saver import sanitize_filename
...
class S3ArtifactSaver(ArtifactSaver):
    ...
    def save(self, *, artifact, path, filename, content_type):
        ...
        kwargs = {"Bucket": bucket, "Key": _assemble_file_save_path(path, filename), "Body": artifact}
        if content_type:
    ...
    def path_for_artifact(self, suite_name, test_name, run_id):
        ...
        return "%s/artifacts/%s/%s" % (run_id, sanitize_filename(suite_name), sanitize_filename(test_name))
    ...
    def on_artifact_created(
        ...
        **kwargs,
        ...
        filename = self.filename_for_artifact(
            artifact_group=artifact_group, artifact_type=artifact_type, mime_type=artifact_mime_type
        )
        log.debug(f"Saving artifact {filename}")
        self.save(artifact=artifact, path=path, filename=filename, content_type=artifact_mime_type)

    def on_report_created(self, report_filename, report_content, run_id, **kwargs):
        log = logger_for_module(__name__)
        log.debug(f"Saving report {report_filename}")
        self.save(artifact=report_content, path=run_id, filename=report_filename, content_type=None)
