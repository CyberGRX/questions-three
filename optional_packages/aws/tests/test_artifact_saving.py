...
def publish_artifact(artifact="spam", **kwargs):
    EventBroker.publish(event=TestEvent.artifact_created, artifact=artifact, **kwargs)


def publish_report(filename="spam", content="spam"):
    EventBroker.publish(event=TestEvent.report_created, report_filename=filename, report_content=content)
...
class TestArtifactSaving(TestCase):
    ...
    def test_complains_if_bucket_not_configured(self):
        ...
        publish_artifact()
        assert 0 == len(self.put_spy.call_history), "Detected an unexpected PUT when the bucket was not configured"
    ...
    def test_saves_under_configured_prefix_object_name_and_run_id(self):
        ...
        publish_artifact()
        prefix_object, run_id, kind, suite, test, filename = self.extract_key_parts(expected_length=6)
    ...
    def test_saves_artifact_without_suite_to_suiteless_directory(self):
        publish_artifact()
        run_id, kind, suite, test, filename = self.extract_key_parts()
        expect(suite).to(equal("_no_suite_supplied"))

    def test_saves_artifact_with_suite_only_to_testless_directory(self):
    ...
    def test_removes_unsafe_chars_from_test_name(self):
        ugly = "I' f!@#$0-_%\\^&*/()?><\",;:`blah+\t\n=.pas"
        pretty = "".join(map(lambda c: c if c in ascii_letters + digits + "-_." else "_", ugly))
    ...
    def test_sends_content_type_if_supplied(self):
    ...
    def test_puts_report_under_configured_bucket(self):
        bucket = "Richard"
        self.context.set_env(S3_BUCKET_FOR_ARTIFACTS=bucket)
        publish_report()
        expect(self.put_spy["Bucket"]).to(equal(bucket))

    def test_puts_report_directly_under_run_id(self):
    ...
    def test_puts_report_content(self):
