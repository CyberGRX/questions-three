...
class TestScreenshotOnFailure(TestCase):
    ...
    def artifact_event_kwargs(self):
        screenshot_events = [
            e for e in self.artifact_events
            if 'artifact_type' in e.keys()
            and e['artifact_type'] == 'screenshot']
    ...
    def test_publishes_screenshot_on_suite_erred(self):
        ...
        expect(self.artifact_event_kwargs()).to(
            contain_key_with_value('suite_name', suite_name))

    def test_publishes_screenshot_on_test_erred_with_test_name(self):
        test_name = 'Tom'
        EventBroker.publish(
            event=TestEvent.test_erred, test_name=test_name)
        expect(self.artifact_event_kwargs()).to(
            contain_key_with_value('test_name', test_name))

    def test_publishes_screenshot_on_test_failed_with_test_name(self):
        test_name = 'Robin'
        EventBroker.publish(
            event=TestEvent.test_failed, test_name=test_name)
        expect(self.artifact_event_kwargs()).to(
            contain_key_with_value('test_name', test_name))
    ...
    def test_screenshot_contains_bytes_from_webdriver(self):
        ...
        expect(self.artifact_event_kwargs()).to(
            contain_key_with_value('artifact', expected))

    def test_mime_type_is_png(self):
        EventBroker.publish(event=TestEvent.test_failed)
        expect(self.artifact_event_kwargs()).to(
            contain_key_with_value('artifact_mime_type', 'image/png'))

    def test_artifact_type_is_screenshot(self):
        EventBroker.publish(event=TestEvent.test_failed)
        expect(self.artifact_event_kwargs()).to(
            contain_key_with_value('artifact_type', 'screenshot'))

    def test_does_not_take_screenshot_if_browser_is_closed(self):
        self.fake_webdriver.open_windows = 0
        EventBroker.publish(event=TestEvent.test_failed)
        assert not self.artifact_event_kwargs(), \
            'A screenshot event was published inappropriately'
