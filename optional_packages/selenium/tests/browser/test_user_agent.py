...
class TestUserAgent(TestCase):
    ...
    def setUp(self):
        ...
        self.context.set_env(
            BROWSER_LOCATION='local', USE_BROWSER='Chrome')
    ...
    def test_sets_arbitrary_chrome_agent(self):
        ...
        self.context.inject_as_class(
            webdriver.chrome.options.Options, options_spy)
        Browser()
        expect(options_spy.arguments).to(
            contain('user-agent=%s' % expected))
