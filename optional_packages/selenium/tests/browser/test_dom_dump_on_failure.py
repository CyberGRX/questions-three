...
class TestDomDumpOnFailure(TestCase):
    ...
    def caught_dom_dump_events(self):
        return [
            e for e in self.artifact_events_caught
            if 'artifact_type' in e.keys() and 'dom_dump' == e['artifact_type']
            ]
    ...
    def test_publishes_dump_on_suite_erred(self):
        ...
        expect(self.caught_dom_dump_kwargs()).to(
            contain_key_with_value('artifact', self.fake_dump))
    ...
    def test_includes_suite_name_on_test_failed(self):
        suite_name = 'SuiteT'
        EventBroker.publish(
            event=TestEvent.test_failed, suite_name=suite_name)
        expect(self.caught_dom_dump_kwargs()).to(
            contain_key_with_value('suite_name', suite_name))
    ...
    def test_artifact_type_is_dom_dump(self):
        EventBroker.publish(event=TestEvent.suite_erred)
        expect(self.caught_dom_dump_kwargs()).to(
            contain_key_with_value('artifact_type', 'dom_dump'))

    def test_mime_type_is_html(self):
        EventBroker.publish(event=TestEvent.suite_erred)
        expect(self.caught_dom_dump_kwargs()).to(
            contain_key_with_value('artifact_mime_type', 'text/html'))
