...
def launch_executable(local_identifier):
    ...
    if not key:
        raise InvalidConfiguration("$BROWSERSTACK_ACCESS_KEY is not configured")
    proc = dependency(Popen)(
        [config.browserstack_local_binary, key, "--local-identifier", local_identifier], stdout=PIPE, stderr=STDOUT
    )
...
def save_executable(executable):
    ...
    try:
        ...
        if e.errno == errno.ETXTBSY:
            log.warning("Failed to update %s because it is busy" % filename)
        else:
            raise
...
class BrowserStackTunnel:
    def __init__(self):
        ...
        EventBroker.subscribe(event=TestEvent.suite_ended, func=self.on_suite_ended)
    ...
    def _open_new_tunnel(self):
        config = config_for_module(__name__)
        path, filename = dependency(os).path.split(config.browserstack_local_binary)
        ...
        self._tunnel_proc = launch_executable(local_identifier=self.local_identifier)
