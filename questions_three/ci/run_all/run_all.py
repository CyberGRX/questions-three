...
def discover(top_dir):
    ...
    for path, _, files in walk(top_dir):
        scripts += [
            dependency(os).path.join(path, fn) for fn in files if not fn.startswith("_") and fn.endswith(".py")
        ]
    return scripts
...
def _float_or_none(name, value):
    ...
    try:
        ...
        raise TypeError(f"Expected {name} ({value}) to be a number") from e
...
def run_all(path):
    ...
    timeout = _float_or_none(name="$RUN_ALL_TIMEOUT", value=cfg.run_all_timeout)
    if timeout is None:
        expiry = None
    else:
        expiry = dependency(datetime).now() + timedelta(
            seconds=_float_or_none(name="$RUN_ALL_TIMEOUT", value=cfg.run_all_timeout)
        )
    pool = Pool(limit)
